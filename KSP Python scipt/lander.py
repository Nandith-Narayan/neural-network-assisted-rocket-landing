...
def get_inputs(connection):
    ...
    inputs[4] = (flight_info.surface_altitude-13)/500.0 
...
def get_color(weight):
    ...
    color = (max(0,int(-weight*255)),max(0,int(weight*255)),0)
    return color
...
def main():
    ...
    while running:
        ...
        control.throttle = ((outputs[0][0]+1))
        if outputs[1][0] > 0.5:
            control.sas_mode = connection.space_center.SASMode.retrograde
        elif outputs[1][0] > 0.5:
            control.sas_mode = connection.space_center.SASMode.radial
        elif outputs[1][0] > 0.5:
